...
if (spillType==1){
  ...
  # indOrder <- lapply(ind1,function(x,y=ind2)which(x==y))
  # indOrder <- unlist(indOrder)
...
if (spillType==2){
  #resolve flight and state information with city level
  dist <- cov2$`Number Incoming Passengers 2013 Oct`
  norm.dist <- (dist - min(dist))/ (max(dist) - min(dist))
  geom.dist <- exp(-norm.dist)
  dist.merge <- merge(x=data.frame(State=cov2$State, geom.dist), y=subset.store[,c("City","State")], by.x="State", by.y="State",sort=F)
  #get appropriate flight distance to sabah from city/state
...
if (spillType==3){
  ...
  dist[which(dist>0)] <- 1
  geom.dist <- dist
  dist.merge <- merge(x=data.frame(State=cov2$State, geom.dist),
                      y=subset.store[,c("City","State")], by.x="State", by.y="State",sort=F)
  #get appropriate flight distance to sabah from city/state
  #get ordered naming for pre-treatment, non=intervention, spillover group
  ind1 <- colnames(X0)
  #flight data with names
...
if (!CUM){
  ...
  out <- rbind(ub, colMeans(alpha[1:C1,]), lb, p_val)
  out1 <- list(ATE=out,PTE=path,treat=sum(sab),spill=sum(spillover),names=names)
}
...
type1 <- sens(spillType = 1,CUM=T)
type2 <- sens(spillType = 2,CUM=T)
type3 <- sens(spillType = 3,CUM=T)
save.image("C:\\Users\\user\\Documents\\nBox\\NatExpSabah\\submission\\placeboEvents\\sensSpillover.Rdata")
