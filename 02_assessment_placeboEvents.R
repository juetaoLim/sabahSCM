...
# FUNCTION taking dates as not given week ago#
sens <- function(treatStart,analysisEnd,CUM=T){
...
analysis.end <- analysisEnd   #travel restrictions from Sabah start
treat.start <- treatStart
...
out1 <- list(ATE=out,PTE=path,treat=sum(sab),spill=sum(spillover))
}
...
sens1 <- sens(treatStart = "2020-08-29",analysisEnd = "2020-09-14",CUM=T)
sens2 <- sens(treatStart = "2020-08-22",analysisEnd = "2020-09-07",CUM=T)
sens3 <- sens(treatStart = "2020-08-15",analysisEnd = "2020-08-31",CUM=T)
sens4 <- sens(treatStart = "2020-08-08",analysisEnd = "2020-08-24",CUM=T)
sens5 <- sens(treatStart = "2020-08-01",analysisEnd = "2020-08-17",CUM=T)

save.image("C:\\Users\\user\\Documents\\nBox\\NatExpSabah\\submission\\placeboEvents\\pvalsSpillover.Rdata")
